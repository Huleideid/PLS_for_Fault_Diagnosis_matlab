...
%% pls建模
A = 6;
[T,P,Q,B,W,R]=pls_NIPALS(X_train,Y_train,A);
R = W*inv(P'*W);
n1=size(X_train,1); % 训练数据的样本数
Ab=inv(T'*T/(n1-1));
%% 计算控制限 Control Limit
alpha=0.95;%显著性水平
T2_lim=A*(n1*n1-1)/(n1*(n1-A))*finv(alpha,A,(n1-A)); %T2控制限
 for i=1:n1 
 ...
 mio1=mean(SPE_old);
 V1=2*mio1^2/S1; 
 Q_lim=S1/(2*mio1)*chi2inv(alpha,V1); %SPE控制限
%% 计算监控指标
n2=size(X_test,1); 
for i=1:n2 
    t_new=R'*X_test(i,:)'; 
...
set(gcf,'color','white')
subplot(2,1,1)
plot(t,T2,t,ones(n2,1)*T2_lim,'r--'); 
xlabel('Sample Number'); 
ylabel('T^2');
legend('Hotelling''s T^2','95% upper limit','Location','best')
subplot(2,1,2)
plot(t,SPE,t,ones(n2,1)*Q_lim,'r--');
xlabel('Sample Number'); 
ylabel('SPE'); 
